...
export default function LandingPage() {
  ...
  return (
    <div className="relative min-h-screen bg-center transition-all duration-500"
      style={{
    ...
    >
      {/* ✅ Overlay faqat background uchun ishlaydi */}
      <div className="absolute inset-0 bg-black/50 z-0"></div>

      {/* ✅ Barcha elementlar overlay ustida turishi uchun z-10 qo‘shildi */}
      <div className="z-10 container">
        {/* ✅ Navbar z-50 qilib overlay ustiga chiqildi */}
        <nav className="flex justify-between items-center px-10 bg-transparent text-white relative z-50">
          <ul className="hidden md:flex space-x-6">
            {['Kompaniya haqida', 'Loyihalar', 'Yangiliklar', 'Kontakt', 'Vakansiyalar'].map((item, index) => (
          ...
          <div className="flex items-center gap-[40px]">
            <div className="relative z-50">
              <button
                className="flex leading-[29px] font-medium items-center gap-[9px] cursor-pointer transition-all duration-700"
                onClick={() => setIsDropdownOpen(!isDropdownOpen)}
              >
                <span>{language}</span>
                <span className={`${isDropdownOpen ? 'rotate-180' : 'rotate-0'}`}>
                  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                    <path d="M12.0001 13.1714L16.9499 8.22168L18.3641 9.63589L12.0001 15.9999L5.63623 9.63589L7.05044 8.22168L12.0001 13.1714Z" fill="white"/>
                  </svg>
                </span>
              </button>
                ...
                <ul className="absolute top-full left-0 mt-2 bg-white text-black rounded-lg shadow-md w-28">
                  {['Uz', 'Ru', 'En'].map((lang, i) => (
                    <li
                      ...
                      className={`px-4 py-2 hover:bg-gray-200 cursor-pointer ${i === 0 ? 'rounded-t-lg' : i === 2 ? 'rounded-b-lg' : ''}`}
                    >
                      {lang}
                    </li>
                  ))}
        ...
        <div className="absolute bottom-50 left-1/2 transform -translate-x-1/2 text-center flex flex-col items-center text-white z-10">
          <h1 className="text-[60px] leading-[70px] font-bold w-[100%]" style={{ fontFamily: 'Playfair Display' }}>
            Qurilish kompaniyasi nomi
          </h1>
          <p className="text-[20px] mt-[13px]">Qurilish Toshkent shahrida joylashgan</p>
        ...
        <div className="flex justify-center items-center space-x-3 mt-10 absolute bottom-10 left-1/2 transform -translate-x-1/2 z-10">
          <button onClick={prevSlide} className="text-white text-2xl hover:text-gray-400 transition cursor-pointer">
          ...
          {backgrounds.map((_, index) => (
            <button
              ...
              className={`relative rounded-full transition-all duration-300 cursor-pointer ${
                activeIndex === index ? 'w-[18px] h-[18px] bg-[#D18202]' : 'w-[12px] h-[12px] bg-[#D1820233] border border-[#D18202]'
              }`}
            ></button>
          ))}

          <button onClick={nextSlide} className="text-white text-2xl hover:text-gray-400 transition cursor-pointer">
